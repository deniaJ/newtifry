...
import com.newtifry.android.database.NewtifryDatabaseAdapter;
...
public class MessageList extends SherlockListActivity
{
	...
	public void refreshView()
	{
		...
		Cursor cursor = NewtifryMessage.FACTORY.cursorList(this, this.getSource());
		this.startManagingCursor(cursor);
		SimpleCursorAdapter adapter = new SimpleCursorAdapter(
				this,
				R.layout.message_list_row,
				cursor,
				new String[] { NewtifryDatabaseAdapter.KEY_TITLE, NewtifryDatabaseAdapter.KEY_TIMESTAMP, NewtifryDatabaseAdapter.KEY_SEEN },
	...
	public NewtifrySource getSource()
	{
		if( this.source == null )
		{
			...
			if( sourceId > 0 )
			{
				this.source = NewtifrySource.FACTORY.get(this, sourceId); 
			}
		}
	...
	public boolean onOptionsItemSelected( MenuItem item )
	{
		switch( item.getItemId() )
		{
			case android.R.id.home:
				Intent intent = new Intent(this, Newtifry.class);
	...
	public void deleteAll( boolean onlySeen )
	{
		...
		NewtifryMessage.FACTORY.deleteMessagesBySource(this, source, onlySeen);
		updateNotifications();
	}
	
	public void markAllAsSeen()
	{
		NewtifryMessage.FACTORY.markAllAsSeen(this, this.getSource());
		updateNotifications();
	}
	...
	private class MessageViewBinder implements SimpleCursorAdapter.ViewBinder
	{
		public boolean setViewValue( View view, Cursor cursor, int columnIndex )
		{
			...
			if( columnIndex == cursor.getColumnIndex(NewtifryDatabaseAdapter.KEY_TIMESTAMP) )
			{
				TextView timestamp = (TextView) view;
				try
				{
					timestamp.setText(NewtifryMessage.formatUTCAsLocal(NewtifryMessage.parseISO8601String(cursor.getString(cursor.getColumnIndex(NewtifryDatabaseAdapter.KEY_TIMESTAMP)))));
				}
			...
			if( columnIndex == cursor.getColumnIndex(NewtifryDatabaseAdapter.KEY_TITLE) )
			{
				TextView title = (TextView) view;
				title.setText(cursor.getString(cursor.getColumnIndex(NewtifryDatabaseAdapter.KEY_TITLE)));
				
				if( cursor.getLong(cursor.getColumnIndex(NewtifryDatabaseAdapter.KEY_SEEN)) == 0 )
				{
