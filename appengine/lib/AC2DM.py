...
class AC2DM:
	...
	def send_to_google(self, params):
		...
		logging.debug("Message : " + urllib.urlencode(params))
		if result.status_code != 200:
			logging.error('Unable to send message to Google: Status code ' + str(result.status_code) + ' body ' + result.content)
		else:
			logging.debug("Send message " + result.content + " code : " + str(result.status_code))
	...
	def notify_source_change(self, source, device):
		...
		params['type'] = "sourcechange"
		params['id'] = source.key().id()
		params['device_id'] = str(device.key().id())
	...
	def send(self, message, device):
		...
		logging.debug("Deviceid : " + device.deviceKey)
		result = self.send_to_google(params)
		...
		if result.status_code == 200:
			...
			mail.send_mail(sender="Notifry <ar.laurent@gmail.com>", to="Notifry <ar.laurent@gmail.com>", subject="Error sending message", body=errorMessage)
			return False
